...
internal sealed class NbtElementConverter(NbtSerializer serializer) : NbtConverter<Dom.NbtElement>
{
    ...
    public override Dom.NbtElement ReadPayload(Stream stream) => throw new InvalidOperationException("A dynamic NBT tag type is required.");

    public override Dom.NbtElement ReadPayload(Stream stream, NbtTagType actualType) => actualType switch
    {
        ...
        NbtTagType.Compound => ReadCompound(stream),
    ...
    public override void WritePayload(Stream stream, Dom.NbtElement? value)
    {
        switch (value)
        {
            ...
            case NbtList item: WriteList(stream, item); break;
            case NbtCompound item: WriteCompound(stream, item); break;
    ...
    private NbtList ReadList(Stream stream)
    {
        ...
        for (int index = 0; index < count; index++) values[index] = ReadPayload(stream, elementType);
        return new NbtList(values);
    }

    private void WriteList(Stream stream, NbtList list)
    {
    ...
    private NbtCompound ReadCompound(Stream stream)
    {
        ...
        while (serializer.ReadTagType(stream) is { } type && type != NbtTagType.End)
        {
            ...
            if (!result.TryAdd(name, ReadPayload(stream, type))) throw new InvalidDataException($"Duplicate NBT key '{name}'.");
        }
        return new NbtCompound(result);
    }

    private void WriteCompound(Stream stream, NbtCompound compound)
    {
